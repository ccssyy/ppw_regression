...
def conv_op(input_op, name, kh, kw, n_out, sh, sw):
    ...
    with tf.name_scope(name) as scope:
        ...
        tf.add_to_collection('losses', tf.contrib.layers.l2_regularizer(1.0)(kernel))
        conv = tf.nn.conv2d(input_op, kernel, [1, sh, sw, 1], padding='SAME')
        bias_init_val = tf.constant(1.0, shape=[n_out], dtype=tf.float32)
...
def fc_op(input_op, name, n_out):
    ...
    with tf.name_scope(name) as scope:
        ...
        tf.add_to_collection('losses', tf.contrib.layers.l2_regularizer(0.5)(kernel))
        biases = tf.Variable(tf.constant(1.0, shape=[n_out], dtype=tf.float32), name='b')
...
def get_segment_benchmack(name):
    with tf.name_scope(name) as scope:
        segment_benchmack = tf.get_variable(scope, shape=[2885],
                                            dtype=tf.float32,
                                            initializer=tf.zeros_initializer(), trainable=False)
        return segment_benchmack
...
def inference_op(input_op, keep_prob):
    ...
    logits = fc_op(fc7_drop, name='fc8', n_out=1)

    return logits, get_segment_benchmack('benchmack')
